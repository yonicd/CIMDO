...
#    selectInput("df", "Measure Type",names(to.shiny),selected=names(to.shiny)[1]),
    selectInput(inputId = "DESC", label = "Measure",choices = levels(to.shiny$DESC),selected=levels(to.shiny$DESC)[1]),
    uiOutput("VARIABLE"),
    ...
    selectInput('var_x', 'X variable', c('None', names(to.shiny)[-8],"TO.DEFAULT"),selected = names(to.shiny)[1]),
    selectInput('var_y', 'Y variable', c('None', names(to.shiny)[-1],"TO.DEFAULT"),selected = names(to.shiny)[8]),
    selectInput('var_fill', 'Group', c('None', names(to.shiny)[-1],"TO.DEFAULT"),selected = names(to.shiny)[7]),
